...
test.describe("PATCH API tests", () => {
    ...
    test("1 . PATCH API request using static body", async ({ request }) => {
        ...
        const deleteAPI_response = await request.patch(`${bURL}/booking/${bId}`, {
            headers:{
                "Content-Type":"application/json",
                "Cookie":`token=${token_number}`
            }
        })
        console.log(deleteAPI_response)

        // assertions
        expect(deleteAPI_response.status()).toEqual(200)
        expect(deleteAPI_response.statusText()).toBe('OK')
